...
def compress(image_file, image, progress, target_size=49 * 1024 * 1024, compression_level=4):
    ...
    if not os.path.exists(image_file) or os.path.getsize(image_file) > optimized_size:
        progress.write(f"Saving image to {image_file}")
        # Save the image
   ...
   # Read the image
    ...
    if os.path.exists(upscaled_filename):
        ...
        if up.shape[1] == args.target_resolution or up.shape[0] == args.target_resolution:
            ...
            image = up
    compress(image_file, image, progress, args.target_size)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        todo = []
        directories = random.sample(directories, 25)
        priority_queue = glob(use_path + "/../To Be Upscaled Priority Queue" + os.sep + "*", recursive=False)
        directories.extend(priority_queue)
        for directory in directories:
            #rename_files(directory)
            files = glob(directory + os.sep + "*.*", recursive=False)
            files = random.sample(files, min(25, len(files)))
            for file in files:
                todo.append(os.path.join(directory, file))


        to_process = tqdm.tqdm(todo, desc="Processing images", unit="images", total=len(todo))
